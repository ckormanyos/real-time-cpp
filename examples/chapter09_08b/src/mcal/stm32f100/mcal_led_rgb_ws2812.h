...
//
  ...
  // The industry-standard RGB LED on the board is of type ws2812b.
  ...
  public:
    ...
    {
      // In this design, all of the RGB LEDs in the chain are
      // animated with the same color pattern. Therefore, we
      // push the color to each RGB LED using a trivial loop.

      for(unsigned i { UINT8_C(0) }; i < led_count(); ++i)
      {
        push_color();
      }
    }
  ...
  #define MCAL_LED_RGB_WS2812_NOPS_03 { asm volatile("nop"); asm volatile("nop"); asm volatile("nop"); }
  #define MCAL_LED_RGB_WS2812_NOPS_06 { MCAL_LED_RGB_WS2812_NOPS_03 MCAL_LED_RGB_WS2812_NOPS_03 }
  #define MCAL_LED_RGB_WS2812_NOPS_09 { MCAL_LED_RGB_WS2812_NOPS_06 MCAL_LED_RGB_WS2812_NOPS_03 }
  ...
  template<typename PortPinType, const unsigned LedCount>
  auto led_rgb_ws2812<PortPinType, LedCount>::push_color() -> void
  {
    using base_class_type = mcal::led::led_rgb_base;

    const std::uint_fast8_t red   { static_cast<std::uint_fast8_t>(base_class_type::get_hue_r()) };
    const std::uint_fast8_t green { static_cast<std::uint_fast8_t>(base_class_type::get_hue_g()) };
    const std::uint_fast8_t blue  { static_cast<std::uint_fast8_t>(base_class_type::get_hue_b()) };
