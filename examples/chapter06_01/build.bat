...
@rem Usage example A,
@rem cd "C:\Users\User\Documents\Ks\uC_Software\Boards\real-time-cpp\examples\chapter06_01"
@rem build.bat "C:\Users\User\Documents\Ks\uC_Software\Boards\real-time-cpp\examples\chapter06_01\tools\Util\msys64\usr\local\gcc-11.2.0-avr\bin" avr
...
@echo.Clean  bin directory : bin\*.o bin\chapter06_01*.*
@if exist bin\*.o del /Q bin\*.o
@if exist bin\chapter06_01*.* del /Q bin\chapter06_01*.*
@echo.
...
@echo.Link     : objects to bin/chapter06_01.elf
@%TOOL_PATH%\%TOOL_PREFIX%-g++ -x none -mrelax -nostartfiles %CFLAGS% %CPPFLAGS% %CINCLUDES% -Wl,--gc-sections -Wl,-Ttarget/micros/avr/make/avr.ld,-Map,bin/chapter06_01.map bin/app_benchmark.o bin/app_led.o bin/mcal.o bin/mcal_gcc_cxx_completion.o bin/mcal_cpu.o bin/mcal_gpt.o bin/mcal_irq.o bin/mcal_led.o bin/mcal_osc.o bin/mcal_port.o bin/mcal_wdg.o bin/os.o bin/os_task_control_block.o bin/sys_idle.o bin/sys_mon.o bin/sys_start.o bin/crt0.o bin/crt0_init_ram.o bin/crt1.o bin/int_vect.o -o bin/chapter06_01.elf

@echo.
@echo.Extract  : executable hex file : from bin/chapter06_01.elf
@%TOOL_PATH%\%TOOL_PREFIX%-objcopy -O ihex bin/chapter06_01.elf bin/chapter06_01.hex

@echo.Extract  : assembly list file  : from bin/chapter06_01.elf
@%TOOL_PATH%\%TOOL_PREFIX%-objdump --disassemble bin/chapter06_01.elf > bin/chapter06_01.lss

@echo.Extract  : size information    : from bin/chapter06_01.elf
@%TOOL_PATH%\%TOOL_PREFIX%-size -A -t bin/chapter06_01.elf > bin\chapter06_01_size.txt

@echo.Extract  : name information    : from bin/chapter06_01.elf
@%TOOL_PATH%\%TOOL_PREFIX%-nm --numeric-sort --print-size bin/chapter06_01.elf > bin\chapter06_01_nm.txt

@echo.Extract  : demangled names     : from bin/chapter06_01.elf
@%TOOL_PATH%\%TOOL_PREFIX%-nm --numeric-sort --print-size bin/chapter06_01.elf | %TOOL_PATH%\%TOOL_PREFIX%-c++filt > bin\chapter06_01_cppfilt.txt

dir .\bin\chapter06_01.elf .\bin\chapter06_01.hex

if not exist .\bin\chapter06_01.elf exit 1
if not exist .\bin\chapter06_01.hex exit 1
