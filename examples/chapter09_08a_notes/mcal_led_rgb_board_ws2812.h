...
//
  ...
  private:
    ...
    auto set_color_rgb() -> void override
    {
      const std::uint8_t red   = base_class_type::get_color_rgb().get_r();
      const std::uint8_t green = base_class_type::get_color_rgb().get_g();
      const std::uint8_t blue  = base_class_type::get_color_rgb().get_b();
