                                ...
                                Crc32_Autosar_Context_Type* Crc_Context)
{
  ...
  for(LoopCnt = 0U; LoopCnt < DataLength; ++LoopCnt)
  {
    ...
    static const uint32_t Crc32_Autosar_Table[256U] MY_PROGMEM =
    {
    ...
    const uint_fast8_t DataIndex =
      (uint_fast8_t) ((uint8_t) (  Crc_Context->Crc_Value
                                 ^ (uint32_t) DataIn[LoopCnt]));

    const uint32_t TableValue =
      mcal_cpu_read_program_memory_dword((const uint8_t*) &Crc32_Autosar_Table[DataIndex]);
