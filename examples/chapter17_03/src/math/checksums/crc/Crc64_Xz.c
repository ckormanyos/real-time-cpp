...
void Crc64_Xz_Initialize(Crc64_Xz_Context_Type* Crc_Context)
{
  ...
  Crc_Context->Crc_Value = UINT64_C(0xFFFFFFFFFFFFFFFF);
}
                           ...
                           Crc64_Xz_Context_Type* Crc_Context)
{
  ...
  for(LoopCnt = 0U; LoopCnt < DataLength; ++LoopCnt)
  {
    ...
    static const uint64_t Crc64_Xz_Table[256U] MY_PROGMEM =
    {
    ...
    const uint_fast8_t DataIndex =
      (uint_fast8_t) ((uint8_t) (  Crc_Context->Crc_Value
                                 ^ (uint64_t) DataIn[LoopCnt]));

    const uint64_t TableValue =
      mcal_cpu_read_program_memory_qword((const  uint8_t*) &Crc64_Xz_Table[DataIndex]);
