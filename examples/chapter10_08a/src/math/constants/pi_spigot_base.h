...
//
  ...
  protected:
    ...
    static constexpr std::uint32_t d_init = pow10_maker(loop_digit) / UINT32_C(5);
  ...
  protected:
    ...
    void do_extract_digit_group(OutputIteratorType output_first)
    {
      ...
      const std::uint32_t d_div10(static_cast<std::uint32_t>(my_d / pow10_maker(loop_digit)));
      const std::uint32_t d_mod10(static_cast<std::uint32_t>(my_d % pow10_maker(loop_digit)));
      ...
      std::uint32_t scale10 = pow10_maker(loop_digit - UINT32_C(1));
