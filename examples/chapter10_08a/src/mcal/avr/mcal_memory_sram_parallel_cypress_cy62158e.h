  ...
  template<typename sram_parallel_port_addr04_to_19_type,
  ...
  public:
    memory_sram_parallel_cypress_cy62158e() : my_word16_addr_04_to_19(0U)
    {
      ...
      sram_parallel_port_pin_ce1not_type::set_pin_high();
      sram_parallel_port_pin_ce2____type::set_pin_low();
      sram_parallel_port_pin_oe_not_type::set_pin_high();
      sram_parallel_port_pin_we_not_type::set_pin_high();

      sram_parallel_port_pin_ce1not_type::set_direction_output();
      sram_parallel_port_pin_ce2____type::set_direction_output();
      sram_parallel_port_pin_oe_not_type::set_direction_output();
      sram_parallel_port_pin_we_not_type::set_direction_output();

      // Set the microcontroller 8-bit port word to input.
      // This is the port that is used for data read/write.
      // the default direction.
      sram_parallel_port_data08_type::set_direction_input();
      ...
      sram_parallel_port_addr04_to_19_type::set_port(UINT16_C(0));
      sram_parallel_port_addr04_to_19_type::set_direction_output();
    ...
    void write(const std::uint32_t address_to_write, const std::uint8_t source_byte_to_write)
    {
      ...
      {
        sram_parallel_port_addr04_to_19_type::set_port(input_word16_addr_04_to_19);

        my_word16_addr_04_to_19 = input_word16_addr_04_to_19;
      }

      // Assert the control pins to select the memory device.
      sram_parallel_port_pin_ce2____type::set_pin_high();
      sram_parallel_port_pin_ce1not_type::set_pin_low();

      // Set the microcontroller port value (i.e., of port_address_read_write_data)
      sram_parallel_port_data08_type::set_port(source_byte_to_write);
      ...
      sram_parallel_port_pin_we_not_type::set_pin_low();
                 ...
                 const std::uint8_t* source_input_to_write)
    {
      ...
      for(std::size_t i = 0U; i < count; ++i)
      {
        ...
        // Set the microcontroller port value (i.e., of port_address_read_write_data)
        sram_parallel_port_data08_type::set_port(*(source_input_to_write + i));
    ...
    std::uint8_t read(const std::uint32_t address_to_read)
    {
      ...
      const std::uint8_t byte_to_read = sram_parallel_port_data08_type::read_port();
                      ...
                      std::uint8_t* dest_to_store_read_data)
    {
      ...
      *dest_to_store_read_data = sram_parallel_port_data08_type::read_port();
      ...
      for(std::size_t i = 1U; i < count; ++i)
      {
        ...
        *(dest_to_store_read_data + i) = sram_parallel_port_data08_type::read_port();
      }
