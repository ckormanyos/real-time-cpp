...
namespace
{
  ...
  using sys_idle_pi_spigot_input_type       = std::array<std::uint32_t,
                                                         sys_idle_pi_spigot_pi_spigot_type::get_input__static_size()>;

  using sys_idle_pi_spigot_value_type       = std::array<std::uint8_t,
                                                         sys_idle_pi_spigot_pi_spigot_type::get_output_static_size()>;

  sys_idle_pi_spigot_input_type               sys_idle_pi_spigot_input MCAL_MEMORY_DRAM_NOINIT_POOL;
  sys_idle_pi_spigot_value_type               sys_idle_pi_spigot_value MCAL_MEMORY_DRAM_NOINIT_POOL;

  sys_idle_pi_spigot_pi_spigot_type           sys_idle_pi_spigot_object;

  std::uintptr_t                              sys_idle_pi_spigot_pi_digits;
  std::uintptr_t                              sys_idle_pi_spigot_pi_digits_100th_pi_digits_counter;
...
void sys::idle::pi_spigot_run()
{
  if(sys_idle_pi_spigot_object.is_finished())
  {
    ...
    const char* p_end = util::baselexical_cast(sys_idle_pi_spigot_rounds_counter, pstr_lcd_line1);

    mcal::lcd::lcd0().write_n(pstr_lcd_line1, std::uint_fast8_t(p_end - (const char*) pstr_lcd_line1), 1U, false);
  ...
  std::uintptr_t new_digit_count = 0U;
  ...
  while(sys_idle_pi_spigot_pi_digits != sys_idle_pi_spigot_object.get_output_digit_count())
  {
    ...
    const std::uintptr_t pi_spigot_digits_100th_digit_position =
      std::uintptr_t(sys_idle_pi_spigot_pi_digits_100th_pi_digits_counter * UINT32_C(100));

      const std::uint8_t next_digit_value =
        *(sys_idle_pi_spigot_value.cbegin() + std::ptrdiff_t(sys_idle_pi_spigot_pi_digits));
    ...
    if(sys_idle_pi_spigot_pi_digits == pi_spigot_digits_100th_digit_position)
    {
      const std::uint8_t next_100th_digit_cntrl =
        *(  math::constants::pi_digits_100th_pi_digits<std::uintptr_t(UINT32_C(1012))>::pi_digits_control()
          + std::ptrdiff_t(sys_idle_pi_spigot_pi_digits_100th_pi_digits_counter));
