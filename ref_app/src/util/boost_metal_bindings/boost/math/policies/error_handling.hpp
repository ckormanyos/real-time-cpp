...
{
public:
   evaluation_error(const std::string) { }
};

class rounding_error
{
public:
   rounding_error(const std::string&) { }
};
#else
...
{
#if defined(BOOST_NO_RTTI) || defined(BOOST_MATH_DISABLE_ERROR_HANDLING)
   return "unknown";
#else
...
void raise_error(const char* pfunction, const char* pmessage)
{
#if defined(BOOST_MATH_DISABLE_ERROR_HANDLING)
  static_cast<void>(pfunction);
  static_cast<void>(pmessage);
#else
  if(pfunction == nullptr)
     pfunction = "Unknown function operating on type %1%";
  if(pmessage == nullptr)
     pmessage = "Cause unknown";
...
inline constexpr T raise_domain_error(const char*, const char*, const T&, const Policy&) noexcept(is_noexcept_error_policy<Policy>::value && BOOST_MATH_IS_FLOAT(T))
{
   return T();
}
#else
