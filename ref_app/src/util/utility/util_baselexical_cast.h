...
//
  ...
  OutputIterator baselexical_cast(const UnsignedIntegerType& u, OutputIterator OutFirst)
  {
    ...
    do
    {
      ...
      output_value_type c = output_value_type(x % BaseRepresentation);
      ...
      {
        c = output_value_type(  (UpperCase ? (output_value_type) 'A' : (output_value_type) 'a')
                              + (output_value_type) (output_value_type(c - (output_value_type) 0xA)));
      }
