#ifndef NON_TEMPLATE_UINT256_T_2018_10_01_H_
  #define NON_TEMPLATE_UINT256_T_2018_10_01_H_
  ...
  namespace std
  {
    ...
    template<> class numeric_limits<wide_integer::non_template::uint256_t>;
  }
  ...
  public:
    ...
    using ushort_type = std::uint64_t;
    using ularge_type = wide_integer::non_template::uint128_t;
    ...
    uint256_t(const wide_integer::non_template::uint128_t& u)
    {
      values[0U] = detail::make_lo<std::uint64_t, wide_integer::non_template::uint128_t>(u);
      values[1U] = detail::make_hi<std::uint64_t, wide_integer::non_template::uint128_t>(u);

      values[2U] = values[3U] = 0U;
    }
    ...
    uint256_t(const representation_type& rep)
    {
    ...
    uint256_t operator++(int) { uint256_t w(*this); ++(*this); return w; }
    uint256_t operator--(int) { uint256_t w(*this); --(*this); return w; }
    ...
    uint256_t& operator<<=(const int n)
    {
      ...
      else if(n >= int(std::numeric_limits<ularge_type>::digits * 2))
      {
        ...
        // of the uint256_t type, then the result is zero.
        operator=(std::uint16_t(0U));
      }
      else
      {
    ...
    bool operator==(const uint256_t& other) const { return (compare(other) == std::int_fast8_t( 0)); }
    bool operator< (const uint256_t& other) const { return (compare(other) == std::int_fast8_t(-1)); }
    bool operator> (const uint256_t& other) const { return (compare(other) == std::int_fast8_t( 1)); }
    bool operator!=(const uint256_t& other) const { return (compare(other) != std::int_fast8_t( 0)); }
    bool operator<=(const uint256_t& other) const { return (compare(other) <= std::int_fast8_t( 0)); }
    bool operator>=(const uint256_t& other) const { return (compare(other) >= std::int_fast8_t( 0)); }
    ...
    static uint256_t limits_helper_max()
    {
      return uint256_t(
                         {
  ...
  private:
    ...
    void division_knuth_simplified(const uint256_t& other)
    {
      ...
      // of the Knuth long division algorithm.
      {
        ...
        for(local_uint_index_type j = local_uint_index_type(0U); j <= m; ++j)
        {
          ...
          // exceeds the range of uint256_t.
    ...
    std::int_fast8_t compare(const uint256_t& other) const
    {
  ...
  template<> class numeric_limits<wide_integer::non_template::uint256_t> : public wide_integer::non_template::numeric_limits_base<wide_integer::non_template::uint256_t>{ };

  } // namespace std

  namespace wide_integer { namespace non_template {

  inline uint256_t operator+ (const uint256_t& left, const uint256_t& right) { return uint256_t(left).operator+=(right); }
  inline uint256_t operator- (const uint256_t& left, const uint256_t& right) { return uint256_t(left).operator-=(right); }
  inline uint256_t operator* (const uint256_t& left, const uint256_t& right) { return uint256_t(left).operator*=(right); }
  inline uint256_t operator/ (const uint256_t& left, const uint256_t& right) { return uint256_t(left).operator/=(right); }
  inline uint256_t operator% (const uint256_t& left, const uint256_t& right) { return uint256_t(left).operator%=(right); }
  ...
  inline uint256_t operator<<(const uint256_t& left, const int n)            { return uint256_t(left).operator<<=(n); }
  inline uint256_t operator>>(const uint256_t& left, const int n)            { return uint256_t(left).operator>>=(n); }

  } } // namespace wide_integer::non_template

#endif // NON_TEMPLATE_UINT256_T_2018_10_01_H_
