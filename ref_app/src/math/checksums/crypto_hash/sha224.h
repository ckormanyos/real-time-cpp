...
///////////////////////////////////////////////////////////////////////////////
// \license BCP 78
// sha224 can not be redistributed or modified with a license less
...
// This work is an implementation of sha224 that has been specifically
...
// "RFC 4868 HMAC-SHA224, SHA384, and SHA512 in IPsec May 2007".
...
// Note: SHA224, 256, 384, 512 are from RFC 4634, not from RFC 4868
...
#ifndef _SHA224_2013_10_21_H_
  #define _SHA224_2013_10_21_H_
  ...
  class sha224 final : public crypto_hash_base
  {
  public:
    typedef my_count_type                  count_type;
    typedef std::array<std::uint8_t,  28U> result_type_as_bytes;
    typedef std::array<char,          56U> result_type_as_chars;
    typedef std::array<std::uint32_t,  7U> result_type_as_integral_values;
    ...
    sha224(const std::uint8_t* data_bytes, count_type message_length);

    sha224(const char*         data_chars, count_type message_length);

    template<typename unsigned_integer_type> sha224(unsigned_integer_type u);
  ...
  private:
    static const std::uint_least8_t sha224_blocksize = 64U;

    static_assert(sha224_blocksize == static_cast<std::uint_least8_t>(64U),
                  "the block size must exactly equal 64");

    typedef std::array<std::uint8_t,  sha224_blocksize> message_block_type;
    typedef std::array<std::uint32_t, sha224_blocksize> transform_block_type;

    std::array<std::uint32_t,  8U>          message_hash;
  ...
  template <typename my_count_type>
  sha224<my_count_type>::sha224(const std::uint8_t* data_bytes, count_type message_length)
  {
    process_data(data_bytes, message_length);
  }

  template <typename my_count_type>
  sha224<my_count_type>::sha224(const char* data_chars, count_type message_length)
  {
    process_data(static_cast<const std::uint8_t*>(static_cast<const void*>(data_chars)), message_length);
  }
  ...
  sha224<my_count_type>::sha224(unsigned_integer_type u)
  {
    process_data(u);
  }

  template <typename my_count_type>
  sha224<my_count_type>::sha224(const sha224& other) : crypto_hash_base    (other),
  ...
  template <typename my_count_type>
  sha224<my_count_type>& sha224<my_count_type>::operator=(const sha224& other)
  {
    if(this != &other)
    {
  ...
  template <typename my_count_type>
  void sha224<my_count_type>::process_data(const std::uint8_t* data_bytes, count_type message_length)
  {
    ...
    while(message_length != static_cast<count_type>(0U))
    {
      ...
      if(message_block_index == sha224_blocksize)
      {
        perform_algorithm();
  ...
  template<typename my_count_type>
  template<typename unsigned_integer_type>
  void sha224<my_count_type>::process_data(unsigned_integer_type u)
  {
  ...
  template <typename my_count_type>
  void sha224<my_count_type>::perform_algorithm()
  {
    ...
    convert_uint8_input_to_uint32_output_reverse(the_message + sha224_blocksize *  message_index,
                                                 the_message + sha224_blocksize * (message_index + 1U),
    ...
    std::array<std::uint32_t, 8U> hash_tmp = message_hash;
  ...
  template <typename my_count_type>
  void sha224<my_count_type>::finalize()
  {
    message_block_type the_last_message_block;
    std::copy((the_message + sha224_blocksize*message_index),
              (the_message + sha224_blocksize*message_index + message_block_index),
    ...
    if(message_block_index > static_cast<std::uint_least8_t>(sha224_blocksize - 8U))
    {
    ...
    // The sha224 stores the 8 bytes of the bit counter in reverse order,
  ...
  template<typename my_count_type>
  void sha224<my_count_type>::reset()
  {
    message_hash[0U] = UINT32_C(0xC1059ED8);
    message_hash[1U] = UINT32_C(0x367CD507);
    message_hash[2U] = UINT32_C(0x3070DD17);
    message_hash[3U] = UINT32_C(0xF70E5939);
    message_hash[4U] = UINT32_C(0xFFC00B31);
    message_hash[5U] = UINT32_C(0x68581511);
    message_hash[6U] = UINT32_C(0x64F98FA7);
    message_hash[7U] = UINT32_C(0xBEFA4FA4);
  ...
  template<typename my_count_type>
  typename sha224<my_count_type>::result_type_as_bytes sha224<my_count_type>::get_result_as_bytes_and_finalize_the_state()
  {
    ...
    convert_uint32_input_to_uint8_output_reverse(message_hash.data(),
                                                 message_hash.data() + message_hash.size() - 1U,
                                                 result.data());
  ...
  template<typename my_count_type>
  typename sha224<my_count_type>::result_type_as_bytes sha224<my_count_type>::get_result_as_bytes_and_nochange_the_state() const
  {
  ...
  template<typename my_count_type>
  typename sha224<my_count_type>::result_type_as_integral_values sha224<my_count_type>::get_result_as_integral_values_and_finalize_the_state()
  {
    if(!the_result_is_finalized) { finalize(); }
  ...
  template<typename my_count_type>
  typename sha224<my_count_type>::result_type_as_integral_values sha224<my_count_type>::get_result_as_integral_values_and_nochange_the_state() const
  {
    // Make a local copy of the hash object.
    sha224 other(*this);
  ...
  template<typename my_count_type>
  typename sha224<my_count_type>::result_type_as_chars sha224<my_count_type>::get_result_as_chars_and_finalize_the_state()
  {
  ...
  template<typename my_count_type>
  typename sha224<my_count_type>::result_type_as_chars sha224<my_count_type>::get_result_as_chars_and_nochange_the_state() const
  {
    // Make a local copy of the hash object.
    sha224 temp_sha224(*this);

    // Finalize the local copy of the hash object,
    // and return the final result from the copied object.
    return temp_sha224.get_result_as_chars_and_finalize_the_state();
  }

#endif // _SHA224_2013_10_21_H_
