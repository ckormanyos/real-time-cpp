...
///////////////////////////////////////////////////////////////////////////////
// \license BCP 78
// sha384 can not be redistributed or modified with a license less
...
// This work is an implementation of sha384 that has been specifically
...
// "RFC 4868 HMAC-SHA384, SHA384, and SHA512 in IPsec May 2007".
...
// Note: SHA224, 256, 384, 512 are from RFC 4634, not from RFC 4868
...
#ifndef _SHA384_2013_10_21_H_
  #define _SHA384_2013_10_21_H_
  ...
  class sha384 final : public crypto_hash_base
  {
  public:
    typedef my_count_type                  count_type;
    typedef std::array<std::uint8_t,  48U> result_type_as_bytes;
    typedef std::array<char,          96U> result_type_as_chars;
    typedef std::array<std::uint64_t,  6U> result_type_as_integral_values;
    ...
    sha384(const std::uint8_t* data_bytes, count_type message_length);

    sha384(const char*         data_chars, count_type message_length);
    template<typename unsigned_integer_type> sha384(unsigned_integer_type u);
  ...
  private:
    static const std::uint_least8_t sha384_blocksize = 128U;

    static_assert(sha384_blocksize == static_cast<std::uint_least8_t>(128U),
                  "the block size must exactly equal 128");

    typedef std::array<std::uint8_t,  sha384_blocksize> message_block_type;
    typedef std::array<std::uint64_t,              80U> transform_block_type;

    std::array<std::uint64_t,  8U>          message_hash;
  ...
  template <typename my_count_type>
  sha384<my_count_type>::sha384(const std::uint8_t* data_bytes, count_type message_length)
  {
    process_data(data_bytes, message_length);
  }

  template <typename my_count_type>
  sha384<my_count_type>::sha384(const char* data_chars, count_type message_length)
  {
    process_data(static_cast<const std::uint8_t*>(static_cast<const void*>(data_chars)), message_length);
  }
  ...
  sha384<my_count_type>::sha384(unsigned_integer_type u)
  {
    process_data(u);
  }

  template <typename my_count_type>
  sha384<my_count_type>::sha384(const sha384& other) : crypto_hash_base    (other),
  ...
  template <typename my_count_type>
  sha384<my_count_type>& sha384<my_count_type>::operator=(const sha384& other)
  {
    if(this != &other)
    {
  ...
  template <typename my_count_type>
  void sha384<my_count_type>::process_data(const std::uint8_t* data_bytes, count_type message_length)
  {
    ...
    while(message_length != static_cast<count_type>(0U))
    {
      ...
      if(message_block_index == sha384_blocksize)
      {
        perform_algorithm();
  ...
  template<typename my_count_type>
  template<typename unsigned_integer_type>
  void sha384<my_count_type>::process_data(unsigned_integer_type u)
  {
  ...
  template <typename my_count_type>
  void sha384<my_count_type>::perform_algorithm()
  {
    ...
    convert_uint8_input_to_uint64_output_reverse(the_message + sha384_blocksize *  message_index,
                                                 the_message + sha384_blocksize * (message_index + 1U),
    ...
    std::array<std::uint64_t, 8U> hash_tmp = message_hash;
  ...
  template <typename my_count_type>
  void sha384<my_count_type>::finalize()
  {
    message_block_type the_last_message_block;
    std::copy((the_message + sha384_blocksize*message_index),
              (the_message + sha384_blocksize*message_index + message_block_index),
    ...
    if(message_block_index > static_cast<std::uint_least8_t>(sha384_blocksize - 16U))
    {
    ...
    // The sha384 stores the 16 bytes of the bit counter in reverse order,
  ...
  template<typename my_count_type>
  void sha384<my_count_type>::reset()
  {
    message_hash[0U] = UINT64_C(0xCBBB9D5DC1059ED8);
    message_hash[1U] = UINT64_C(0x629A292A367CD507);
    message_hash[2U] = UINT64_C(0x9159015A3070DD17);
    message_hash[3U] = UINT64_C(0x152FECD8F70E5939);
    message_hash[4U] = UINT64_C(0x67332667FFC00B31);
    message_hash[5U] = UINT64_C(0x8EB44A8768581511);
    message_hash[6U] = UINT64_C(0xDB0C2E0D64F98FA7);
    message_hash[7U] = UINT64_C(0x47B5481DBEFA4FA4);
  ...
  template<typename my_count_type>
  typename sha384<my_count_type>::result_type_as_bytes sha384<my_count_type>::get_result_as_bytes_and_finalize_the_state()
  {
    ...
    convert_uint64_input_to_uint8_output_reverse(message_hash.data(),
                                                 message_hash.data() + message_hash.size() - 2U,
                                                 result.data());
  ...
  template<typename my_count_type>
  typename sha384<my_count_type>::result_type_as_bytes sha384<my_count_type>::get_result_as_bytes_and_nochange_the_state() const
  {
  ...
  template<typename my_count_type>
  typename sha384<my_count_type>::result_type_as_integral_values sha384<my_count_type>::get_result_as_integral_values_and_finalize_the_state()
  {
    if(!the_result_is_finalized) { finalize(); }

    result_type_as_integral_values result;

    std::copy(message_hash.begin(),
              message_hash.end() - 2U,
              result.begin());
  ...
  template<typename my_count_type>
  typename sha384<my_count_type>::result_type_as_integral_values sha384<my_count_type>::get_result_as_integral_values_and_nochange_the_state() const
  {
    // Make a local copy of the hash object.
    sha384 other(*this);
  ...
  template<typename my_count_type>
  typename sha384<my_count_type>::result_type_as_chars sha384<my_count_type>::get_result_as_chars_and_finalize_the_state()
  {
  ...
  template<typename my_count_type>
  typename sha384<my_count_type>::result_type_as_chars sha384<my_count_type>::get_result_as_chars_and_nochange_the_state() const
  {
    // Make a local copy of the hash object.
    sha384 temp_sha384(*this);

    // Finalize the local copy of the hash object,
    // and return the final result from the copied object.
    return temp_sha384.get_result_as_chars_and_finalize_the_state();
  }

#endif // _SHA384_2013_10_21_H_
