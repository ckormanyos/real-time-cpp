...
{
  // Increment the 64-bit system tick with 0x10000.
...
mcal::gpt::value_type mcal::gpt::secure::get_time_elapsed()
{
  if(gpt_is_initialized())
  {
    ...
    const timer_register_type   tmr_tick_1 = mcal::reg::reg_access_static<timer_address_type, timer_register_type, mcal::reg::rpi_armtimer_value>::reg_get();
    const mcal::gpt::value_type sys_tick_1 = system_tick;

    // Do the second read of the rpi_arm timer value.
    const timer_register_type   tmr_tick_2 = mcal::reg::reg_access_static<timer_address_type, timer_register_type, mcal::reg::rpi_armtimer_value>::reg_get();

    // Perform the consistency check.
    const mcal::gpt::value_type consistent_microsecond_tick
      = ((tmr_tick_2 < tmr_tick_1) ? mcal::gpt::value_type(sys_tick_1  | timer_register_type(system_timer_reload - tmr_tick_1))
                                   : mcal::gpt::value_type(system_tick | timer_register_type(system_timer_reload - tmr_tick_2)));
