...
static const int8_t esp32_adc2gpio[20] =
{
  36, 37, 38, 39, 32, 33, 34, 35, -1, -1, 4, 0, 2, 15, 13, 12, 14, 27, 25, 26
};

static const esp32_gpioMux_t esp32_gpioMux[GPIO_PIN_COUNT] =
{
  {0x44, 11, 11, 1},
  {0x88, -1, -1, -1},
  {0x40, 12, 12, 2},
...
void pinMode(const uint8_t pin, const uint8_t mode)
{
  ...
  const uint32_t rtc_reg = rtc_gpio_desc[pin].reg;
  ...
  if(mode == ANALOG)
  {
    ...
    reg_val &= ~(  (RTC_IO_TOUCH_PAD1_FUN_SEL_V << rtc_gpio_desc[pin].func)
  ...
  {
    // Pins above 33 can be only inputs.
    if(pin > 33)
    {
      // Unlock gpio.
      return;
...
int digitalRead(const uint8_t pin)
{
  ...
  if(pin < 32)
  {
    result = (GPIO.in >> pin) & 0x1;
  }
  else if(pin < 40)
  {
    result = (GPIO.in1.val >> (pin - 32)) & 0x1;
  }
  else
  {
    result = 0;
